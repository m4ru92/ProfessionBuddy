...
Rationale and sourcing that would otherwise clutter inline comments. Referenced
from the code by feature name (e.g. `-- see DESIGN-NOTES.md`).
...
observed orange 1-25 / yellow 26-50 / green 51-100 / grey 101+.
...
Skinning curve verified vs wow-professions / icy-veins. Mining and herbing
corpses use the same corpse-gather mechanic (Outland mobs are lvl 60-67 -> L*5),
confirmed in-game against the game's own "Requires <skill> (N)" corpse line.
...
Note: `GetSkillLineInfo` returns base rank, not effective (ignores +skill tool/
glove modifiers) — matches the prior cache behaviour, no regression.

## World node tooltips

World herb/ore nodes are GameObjects, not units, so `OnTooltipSetUnit` never
fires for them. Their tooltip is also (re)built by the client over several frames
after it shows, and can be rebuilt again mid-hover, which wipes an appended line
— so a one-shot read or a "once per node" guard fails intermittently ("just the
name"). The robust approach: on a throttled `OnUpdate` (~20 Hz), if the hovered
object is a known node and our line isn't currently present, (re)append it.
...
Regenerated by `tools/gather_db.py` from cmangos-tbc (currently TBCDB 1.11.0). A
...
positives live in `EXCLUDE_SKIN` (currently 7395 Cockroach). The 1.11.0 re-source
removed ~35 false positives the older 1.10.0 data carried (spiders like Giant
...
**Minimap is an incidental surface.** The native Anniversary client routes
minimap node-blip tooltips through `GameTooltip`, so the node hook opportunist-
ically appends to them too. This is a bonus, not a designed surface — it can be
...
Random-property crafted results (e.g. Aquamarine Signet) show a green `<Random
enchantment>` line. In 2.5.x that line is a **crafting-preview** artifact: the
game only renders it via the trade-skill result path (`SetTradeSkillItem`), never
from an item ID or item link. `SetItemByID` and `SetHyperlink("item:"..id)` both
build the plain base tooltip (fixed stats only). PB shows recipes from static
data — including missing ones not in the live trade-skill window — so it can't use
`SetTradeSkillItem`, and therefore **appends the line itself** for known
random-property items (`TSF:AppendRandomEnchantLine`, idempotent so it never
doubles a natively-shown line).
...
Currently 13 items across BS/LW/Eng/JC (the Wild Leather set, Gemmed Copper
