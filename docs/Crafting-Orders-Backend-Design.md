# Backend design pass for crafting orders

Status: design draft (not built). Front-end v1 is complete and solo-tested.
This document covers the networking layer that moves orders between two
...
An order lives only in the requester's SavedVariables. Acting on it solo
(ordering between your own alts) works because both records are on one machine.
The backend makes an order created by Player A appear in Player B's queue. It
also keeps the status in sync as either side acts, surviving the realities below.
...
## 2. What the addon provides (the seam)

The three layers the backend builds on are as follows:
...
| UI | `OrdersPanel.lua` | queues, history, badge | add a "delivery pending" indicator and notifications |

The two logic layers do not know about each other, and should stay that
way. A thin bridge (`OrderSync`) connects them so neither gains a hard
...
WoW addon comms are not a message bus. These constraints dictate the design:

1. **No server-side queue.** If the recipient is offline, the message is gone.
   There is no "deliver later" from Blizzard. Persistence has to be your own.
...
5. **The sender is trustworthy.** Blizzard authenticates the sender name on an
   addon message; you cannot spoof who it came from. Still validate that the
   sender is a party to the order they are mutating.
6. **Clocks.** `time()` is server epoch (UTC based), consistent enough within a
   realm; cross-realm skew is seconds at most. Good enough for tiebreaks, but
   pair it with a per-order version counter.

The single most important consequence: **the order records themselves are the
persistent queue.** Both sides keep the full record in SavedVariables. You do not
need a server to "hold" an order; you need the two copies to reconcile whenever
the two players can next talk. That reconciliation (section 6) is what makes the
whole thing hold up without infrastructure.
...
- `OrderSync.lua` is the bridge module. It owns the outbox, ACK tracking, retry
  triggers, digest reconciliation, conflict rules, and translation between order
  records and wire payloads.
- `Comm.lua` gains ORDER_* cases in its dispatcher that hand off to `OrderSync`.

---

## 5. Wire protocol (ORDER_* message types)

The types below all ride the `Comm:Send` envelope (`_type`, `_ver`, `_from`):

| Type | Direction | Payload | Meaning |
|---|---|---|---|
| `ORDER_NEW` | requester to crafter | full order record | creates an order |
| `ORDER_UPD` | actor to other party | `{id, status, completedBy, version, updatedAt, actor}` | a transition happened |
| `ORDER_ACK` | receiver to sender | `{id, version}` | I received your id@version; clear it from your outbox |
...
- Send at once. Start an ACK watch.
...
Digest reconciliation piggybacks on the contact sync. When two clients sync (the
...
- id present remotely but not locally, or remote version ahead: `ORDER_PULL`
  it and apply.
- terminal locally but not remotely: push the terminal update.

This reconciliation heals **any dropped message, any offline gap, or any missed
transition.** The heal happens the next time the two players sync online. The
order records are the queue; the digest is the diff. No server required.

Ship only 6b and orders stay correct, slower to appear. They catch up on the next
sync rather than at once. 6a is the promptness layer on top. That split gives a
natural MVP boundary (section 10).
...
- **Idempotent apply.** Apply an incoming update only if
  `incoming.version > local.version` (or equal version but a later `updatedAt`).
  Re-delivered messages become no-ops.
- **Terminal stickiness.** Completed, Declined, and Cancelled are final. An
...
- **Concurrent legal moves.** Most states have exactly one legal actor, so true
  races are rare. Take the common one (requester Cancels while crafter Accepts,
  both legal from Pending): a higher version wins. Because Cancel is also legal
  from Accepted, the requester's intent self-heals on the next action, and the
  system converges.
- **The genuine race** (requester Cancels vs crafter MarkCrafted from Accepted):
  ...
  Crafted that cross surface as a soft conflict ("heads up, talk to your
  friend"). Surface it rather than pick silently, because real goods may have
  changed hands. Rare; handled in B5, not the MVP.

---

## 8. Trust and abuse (light, for a friend tool)

- Validate `sender == order.requester` for requester actions and
  `sender == order.crafter` for crafter actions. Drop otherwise.
- Accept `ORDER_NEW` only when it names you as the crafter, from a known contact,
...
- Add a small "delivering..." / "synced" indicator per order, driven by whether
  the order has an outbox entry.
- Notifications already have settings (`orderChatMessages`, `orderSoundOnRequest`
  ...
  `GetActionableCount`, which starts reflecting remote orders for free.

---

## 10. Implementation increments

Each increment is independently shippable and testable. B2 onward needs a second
client for true cross-client testing: an alt on a second account, a guildie, or a
friend. The apply logic stays unit-testable solo via `/script`. The increments
are as follows:

- **B1, the seam.** Add the `OrderSync` module. Add ORDER_* cases to the Comm
  ...
  this wires the plumbing and is verifiable with print stubs.
- **B2, online-to-online propagation.** ORDER_NEW + ORDER_UPD send and apply.
  ...
  net yet (drop a message and the message is lost until B4).
...
- **B4, digest reconciliation.** Piggyback ORDER_DIG / ORDER_PULL on sync. This
  increment is the robustness keystone (heals drops and offline gaps).
- **B5, conflict + notifications polish.** Precedence hardening, soft conflict
  warning, toast/sound via the notification settings.
...
the polish that makes delivery feel fast and friendly.

---

## 11. Open decisions (need your input)

1. **Realm and faction topology of your crafting friends.** This decision is the
   big one.
   - Same realm, same faction: whispers work anytime, the full design applies,
     and B6 is skippable. Simplest and best.
   - Cross-realm: live delivery only works while grouped (constraint 3). The
     design still functions, but B6 becomes necessary and "place an order"
     implies "deliver it next time you are grouped or both synced." Worth knowing
     before B2.
   - Cross-faction: not possible over addon comms at all; would need an
     out-of-band fallback (whisper them). Hopefully not the case.

2. **Delivery ambition for v1.** Recommend MVP = B1+B2+B4 (correct and
   self-healing) and treat B3's fast-delivery polish as a fast follow. Confirm,
   or push for full B1 through B5 in one pass.

3. **A second test client.** Cross-client testing past B1 needs one. Do you have
   a second account or a guildie willing to test, or should I build B2+ against
